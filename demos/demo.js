...
function initPermission(){
 ...
 function run(forceCost=false){const r=roles[$('#permRole').value],q=forceCost?'查看各品牌成本价、库存成本和毛利':$('#permQuery').value,needCost=/成本|毛利|结算/.test(q),needInventory=/库存/.test(q),required=needCost?'成本':needInventory?'库存':'销售',allowed=r.groups.includes(required),gates=[['身份解析',r.name,true],['指标识别',required,true],['字段权限',allowed?'已授权':'未授权',allowed],['数据范围',r.scope,true],['执行闸门',allowed?'允许执行':'拒绝执行',allowed]];$('#permGates').innerHTML=gates.map(([a,b,ok])=>`<div class="gate ${ok?'pass':'block'}"><span>${a}</span><b>${b}</b></div>`).join('');const result=$('#permResult'),status=$('#permStatus');result.classList.toggle('blocked',!allowed);result.innerHTML=allowed?`<b>允许执行</b><span>查询范围已缩小为“${r.scope}”，仅返回授权字段。</span>`:`<b>查询已拦截</b><span>${r.name}没有“${required}”字段组权限，SQL 未进入数据库。</span>`;status.textContent=allowed?'ALLOWED':'BLOCKED';status.className=`pill ${allowed?'ok':'danger'}`;$('#permRows').innerHTML=allowed?`<tr><td>${r.scope}</td><td>¥ ${money(r.scope==='仅云桥店'?128640:846300)}</td><td>${fmt(r.scope==='仅云桥店'?1246:7280)}</td><td>${needCost?'¥ 418,200':'— 无权限字段'}</td></tr>`:'<tr><td colspan="4" class="empty">执行前已拒绝，没有返回数据</td></tr>';$('#permAudit').innerHTML=`<div><b>${allowed?'ALLOW':'DENY'} · ${required}</b><br><small>${new Date().toLocaleTimeString('zh-CN')} · ${r.name}</small></div><div><b>范围：${r.scope}</b><br><small>策略来源：模拟 RBAC + ABAC</small></div><div><b>${allowed?'已生成受控查询':'未生成 SQL'}</b><br><small>${allowed?'字段和范围已写入查询约束':'拒绝原因已写入审计记录'}</small></div>`}
 $('#permRun').onclick=()=>run(false);$('#permCost').onclick=()=>{$('#permQuery').value='查看各品牌成本价、库存成本和毛利';run(true)};run(false)
}
